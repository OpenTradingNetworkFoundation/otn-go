...
func NewTime(t time.Time) Time {
	return Time{t.Round(time.Second)}
}
...
func (b Binary) Marshal(enc *util.TypeEncoder) error {
	if err := enc.EncodeUVarint(uint64(len(b))); err != nil {
		return err
	}
...
func (b Binary) MarshalJSON() ([]byte, error) {
	return []byte(strconv.Quote(hex.EncodeToString(b))), nil
}
...
func (v *Variant) MarshalJSON() ([]byte, error) {
	return json.Marshal([]interface{}{v.Type, v.Value})
}
